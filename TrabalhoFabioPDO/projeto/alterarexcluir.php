...
<form method="post" action="alterarexcluir.php">
    <input type="text" class="col_4" name="q" style="margin:40px 20px;" placeholder="Digite o nome que deseja Alterar ou Excluir">
...
try {
    ...
    if($q != '')
    {
        ...
        foreach($linhas as $linha) {
            ...
            <?php
            ?>
            <td style="padding: 6px 30px 6px 20px;"> <button type="button" class="medium red icon-remove-sign" onclick="window.location.href='tratarexcluir.php?id=<?php echo $linha['id_pessoa'] ?>'">  Excluir</button> </td>
            <?php
            echo '</tr>';
        }
